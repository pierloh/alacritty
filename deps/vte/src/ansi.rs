...
pub trait Handler {
    ...
    /// Handle kitty multi-cursor protocol (CSI > ... SP q).
    ///
    /// Each sub-slice in `params` is a semicolon-separated group with
    /// colon-separated sub-parameters. The first param's first value is the
    /// cursor shape. Subsequent params define coordinates (type 2 = point:
    /// y:x pairs, 1-indexed).
    ///
    /// Spec: https://sw.kovidgoyal.net/kitty/multiple-cursors-protocol/
    fn set_extra_cursors(&mut self, _params: &[&[u16]]) {}
}
...
pub enum ClearMode {
    ...
    Saved,
    /// Clear entire terminal and scrollback (kitty ED 22).
    AllAndSaved,
}
    ...
    T: Timeout,
{
    ...
    fn csi_dispatch(
        ...
        match (action, intermediates) {
            ...
            ('q', [b'>', b' ']) => {
                let mut raw_params = [&[0u16][..]; 32];
                let mut count = 0;
                for p in params.iter() {
                    if count >= raw_params.len() {
                        break;
                    }
                    raw_params[count] = p;
                    count += 1;
                }
                handler.set_extra_cursors(&raw_params[..count]);
            },
            ('q', [b' ']) => {
