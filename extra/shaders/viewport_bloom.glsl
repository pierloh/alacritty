// Animated Glow/Bloom shader for terminal text
// Adapted from bloom shader to create an occasional sweeping shine effect.
...
bool isTargetColor(vec3 rgb) {
  vec3 targetColor = vec3(193.0/255.0, 201.0/255.0, 247.0/255.0);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // SHINE PATTERN
  // false = uniform sine sweep, true = random noise shimmer
  const bool NOISE_PATTERN = true;
  const float NOISE_SCALE = 8.0;
  const float NOISE_SPEED = 1.2;

  float shine;
  if (NOISE_PATTERN) {
    vec2 p = uv * NOISE_SCALE + iTime * NOISE_SPEED * vec2(0.3, 0.7);
    vec2 i = floor(p);
    vec2 f = fract(p);
    f = f * f * (3.0 - 2.0 * f);
    ...
    shine = mix(mix(a, b, f.x), mix(c, d, f.x), f.y) * 0.8;
  } else {
    float sweepSpeed = 0.9;
    float waveFreq = 20.0;
    float t = iTime * sweepSpeed;
    shine = sin(t + uv.x * waveFreq);
    shine = smoothstep(0.0, 1.0, (shine + 1.0) / 2.0) * 0.8;
  }
  ...
  for (int i = 0; i < 24; i++) {
    ...
    float l = lum(c);
    // if (isTargetColor(c.rgb) && l > 0.3) { // (original: only bloom specific color)
    if (l > 0.3) {
